...
public class BubbleDrawer {
    ...
    private void initDefaultBubble(int width) {
        if (mBubbles.size() == 0) {
            mBubbles.add(new CircleBubble(0.30f * width, 0.4f * width, 0.06f * width, 0.022f * width, 0.20f * width,
                    0.0150f, 0x80ffc7c7));
            mBubbles.add(new CircleBubble(0.30f * width, 0.8f * width, -0.15f * width, 0.032f * width, 0.20f * width,
                    0.00600f, 0x85fffc9e));
            mBubbles.add(new CircleBubble(0.30f * width, 1.2f * width, 0.08f * width, -0.015f * width, 0.20f * width,
                    0.00300f, 0x7596ff8f));
            mBubbles.add(new CircleBubble(0.8f * width, 0.4f * width, -0.08f * width, -0.015f * width, 0.20f * width,
                    0.00200f, 0x80c7dcff));
            mBubbles.add(new CircleBubble(0.8f * width, 0.8f * width, -0.06f * width, 0.022f * width, 0.20f * width,
                    0.0150f, 0x70efc2ff));
            mBubbles.add(new CircleBubble(0.8f * width, 1.2f * width, 0.10f * width, 0.050f * width, 0.20f * width,
                    0.0100f, 0x75E99161));
        }
    }
